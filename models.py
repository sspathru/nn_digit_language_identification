...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        t2 = nn.AddBias(t1, self.b1)
        t3 = nn.ReLU(t2)
        t4 = nn.Linear(t3,self.w2)

        return nn.AddBias(t4,self.b2)
    ...
    def train(self, dataset):
        ...
        while loss > 0.02:
            for x, y in dataset.iterate_once(self.batch_size):
                gw1, gb1, gw2, gb2 = nn.gradients(self.get_loss(x,y), [self.w1, self.b1, self.w2, self.b2])
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w5 = nn.Parameter(256,256)
        self.b5 = nn.Parameter(1,256)
        
        self.w6 = nn.Parameter(256,5)
        self.b6 = nn.Parameter(1,5)
    ...
    def run(self, xs):
        ...
        for i in range(len(xs)):
            if i == 0:
                t1 = nn.Linear(xs[0],self.w1)
                t2 = nn.AddBias(t1,self.b1)
                t3 = nn.ReLU(t2)
                t4 = nn.Linear(t3,self.w2)
                t5 = nn.AddBias(t4,self.b2)
                t6 = nn.ReLU(t5)
                t7 = nn.Linear(t6,self.w3)
                t8 = nn.AddBias(t7, self.b3)
            else:
                t1 = nn.Add(nn.Linear(xs[i],self.w1), nn.Linear(t8,self.w4))
                t2 = nn.AddBias(t1,self.b1)
                t3 = nn.ReLU(t2)
                t4 = nn.Linear(t3,self.w2)
                t5 = nn.AddBias(t4,self.b2)
        ...
        a1 = nn.Linear(t8,self.w5)
        a2 = nn.AddBias(a1,self.b5)
        a3 = nn.ReLU(a2)
        a4 = nn.Linear(a3,self.w6)
        
        return nn.AddBias(a4,self.b6)
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < .85:
            for x, y in dataset.iterate_once(self.batch_size):
                gw1, gb1, gw2, gb2, gw3, gb3, gw4, gw5, gb5, gw6, gb6 = nn.gradients(self.get_loss(x,y), [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.w4, self. w5, self.b5, self.w6, self.b6])
                self.w1.update(gw1, self.learning_rate)
                self.w2.update(gw2, self.learning_rate)
                self.w3.update(gw3, self.learning_rate)
                self.w4.update(gw4, self.learning_rate)
                self.w5.update(gw5, self.learning_rate)
                self.w6.update(gw6, self.learning_rate)
                self.b1.update(gb1, self.learning_rate)
                self.b2.update(gb2, self.learning_rate)
                self.b3.update(gb3, self.learning_rate)
                self.b5.update(gb5, self.learning_rate)
                self.b6.update(gb6, self.learning_rate)
